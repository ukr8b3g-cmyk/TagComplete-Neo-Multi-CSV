...
async function getTacExtraNetworkPreviewURL(filename, type) {
    ...
    const previewJSON = await fetchTacAPI(`tacapi/v1/thumb-preview/${encodedFilename}?type=${encodedType}`, true, true);
    if (previewJSON?.url) {
        ...
        if ((await fetch(properURL)).status == 200) {
            ...
            // create blob url
            const blob = await (await fetch(`tacapi/v1/thumb-preview-blob/${encodedFilename}?type=${encodedType}`)).blob();
...
async function updateAnimaCheckpointStatus() {
    if (!TAC_CFG || TAC_CFG.animaArtistPrefix !== "Auto" || !currentModelHash) {

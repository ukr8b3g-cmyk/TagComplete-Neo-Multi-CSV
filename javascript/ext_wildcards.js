    ...
    async parse() {
        ...
        for (let i = 0; i < wcPairs.length; i++) {
            ...
            if (basePath.endsWith(".yaml")) {
                ...
                const params = new URLSearchParams({basepath: basePath, filename: `${fileName}.txt`});
                const rawContent = await fetchTacAPI(`tacapi/v1/wildcard-contents?${params.toString()}`, false);
                const fileContent = String(rawContent || "")
                    .split("\n")
                    .filter(x => x.trim().length > 0 && !x.trimStart().startsWith('#'));  // Remove empty lines and comments
